          ...
          system: `You are a task analysis assistant. Analyze the user's input and categorize it. Return ONLY a JSON object (no markdown, no preamble) with this exact structure:
{
  ...
  "suggestedDeadline": "today" | "this week" | "next week" | "flexible",
  "summary": "one sentence summary of the item",
  "reasoning": "brief explanation of why this category/priority"
}

Categories:
- research: research tasks, reading, learning, investigation
- task: actionable to-dos, commitments, follow-ups
- idea: reflections, potential directions, inspiration
- urgent: time-sensitive, blocking, or critical items`,
          messages: [
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-4 md:p-8">
        <div className="max-w-2xl mx-auto">
          ...
          <form onSubmit={handleCapture} className="mb-8">
            <div className={`bg-white rounded-lg shadow-sm border overflow-hidden transition ${isListening ? 'border-blue-400 ring-2 ring-blue-100' : 'border-slate-200'}`}>
              <textarea
                value={input}
          ...
          <div className="space-y-3">
            {filteredTasks.length === 0 ? (
              ...
              filteredTasks.map(task => (
                <div
                  key={task.id}
                  className={`bg-white rounded-lg shadow-sm border border-slate-200 p-4 transition ${
                ...
                >
                  <div className="flex gap-3 items-start">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className={`text-slate-900 ${task.completed ? 'line-through' : ''}`}>{task.summary}</p>
